...
flags.DEFINE_integer('seed', 0, 'Environment name.')
flags.DEFINE_integer('mode', 0, 'Environment name.')
...
def rational(x: float, a: float, b: float, c: float, d: float, e: float, f: float):
    num = a * x ** 3 + b * x ** 2 + c * x + d
    denom = jnp.abs(e * x ** 2) + jnp.abs(f * x) + 1
    return num/denom

class RationalActor(nn.Module):
...
def log_to_wandb_timestep(rew, timestep):
    wandb.log({f'res/timesteps': timestep, 
               f'res/rews': rew}, step=timestep)
...
def main(_):
    ...
    wandb.init(
        ...
        entity='naumix',
        project='PPO_DMC',
        group=f'{config["ENV_NAME"]}_Variant:{config["MODE"]}',
        name=f'Seed:{config["SEED"]}_AR:{config["ACTION_REPEAT"]}')
    
    #evaluation_results = np.zeros((config["NUM_UPDATES"], config['NUM_SEEDS']))   
    #wandb.define_metric(f'seed{seed}/timesteps')
    #wandb.define_metric(f'seed{seed}/*', step_metric=f'seed{seed}/timesteps')
    
    env = make_env_dmc(config["ENV_NAME"], config["SEED"], config["NUM_ENVS"], 1000, config["ACTION_REPEAT"])
    ...
    if config['MODE'] == 0:
        actor = Actor(env.action_space.shape[-1], activation=config["ACTIVATION"])
    if config['MODE'] == 1:        
        actor = RationalActor(env.action_space.shape[-1], a=0.2645093, b=0.94443867, c=0.97264819, d=-0.13646613, e=0.99920518, f=0.87352966)
    ...
    for update in range(config["NUM_UPDATES"]):
        ...
        return_ = buffer.rewards.mean()*1000/config["ACTION_REPEAT"]
        log_to_wandb_timestep(return_, (update+1)*config["NUM_STEPS"]*config["NUM_ENVS"])
